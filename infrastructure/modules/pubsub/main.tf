...
resource "google_pubsub_subscription" "default" {
  ...
  dynamic retry_policy {
    for_each = try(var.retry_policy[each.key], null) == null ? [] : [""]
    content {
      minimum_backoff = var.retry_policy[each.key].minimum_backoff
      maximum_backoff = var.retry_policy[each.key].maximum_backoff
    }
  }
